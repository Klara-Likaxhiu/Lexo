    ...
    try {
      ...
      if (config.google_client_id) {
        ...
        googleBtn.addEventListener("click", async () => {
          LexoAuthUI.setLoading(googleBtn, true, "Continue with Google", "Signing in…");
          try {
            await window.LexoAuth.signInWithGoogle(config.google_client_id, rememberMeSelector, onSuccess);
            LexoAuthUI.showToast("Signed in with Google!");
            if (onSuccess) onSuccess();
          } catch (error) {
            if (error && error.message) {
              LexoAuthUI.showToast(error.message, "error");
            }
          } finally {
            LexoAuthUI.setLoading(googleBtn, false, "Continue with Google");
          }
        });
      ...
      if (config.apple_client_id) {
        ...
        appleBtn.addEventListener("click", async () => {
          LexoAuthUI.setLoading(appleBtn, true, "Continue with Apple", "Signing in…");
          try {
            await window.LexoAuth.signInWithApple(config.apple_client_id, rememberMeSelector, onSuccess);
            LexoAuthUI.showToast("Signed in with Apple!");
            if (onSuccess) onSuccess();
          } catch (error) {
            ...
            LexoAuthUI.setLoading(appleBtn, false, "Continue with Apple");
          }
        });
...
window.LexoAuthUI = LexoAuthUI;

document.addEventListener("DOMContentLoaded", () => LexoAuthUI.initPasswordToggles());
